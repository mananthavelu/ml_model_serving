...
def train_housing_model(
    ...
    mlflow_run_name = config["model"]["registry"].get(
    ...
    mlflow.sklearn.log_model(
        sk_model=trainer.model, artifact_path="model"
    )  # Log the actual scikit-learn model
    mlflow.end_run()

    logger.info(f"Model saved to {model_output_path}")
    return {"model_type": model_type, "metrics": metrics, "shape": train_df.shape}
